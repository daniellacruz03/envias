...
- **Carlos:** `carlos@envias.com` | `chofer123`
- **Eulogio:** `eulogio@envias.com` | `chofer123`
- **Albert:** `albert@envias.com` | `chofer123`
- **José:** `jose@envias.com` | `chofer123`
- **Mario:** `mario@envias.com` | `chofer123`
- **Emiro:** `emiro@envias.com` | `chofer123`

## Módulos y Rutas
- `/login`: Inicio de sesión seguro y buscador público de guías con redirección por rol.
- `/dashboard`: Panel de logística central con 5 vistas unificadas (`Lista General`, `Agrupado por Ciudad`, `Estado de Choferes` con 3 fases: Almacén, En Ruta, Entregado y visor de fotos de entrega en vivo, `Revisar Guías` para aprobación de escaneos OCR/cámara de choferes, y `Crear Listín` para generación de listines de carga/despacho en formato dual oficial NRO DE GUIA | PIEZAS | DESTINO con editor jerárquico de zonas y exportador a PDF / hoja de impresión), Live Sync cada 3.5s, botón de WhatsApp con cambio automático a `Contactado`, asignación de choferes, selector satelital de mapas con motor de decodificación OLC (Plus Codes), optimizador de rutas por jerarquía de cercanía satelital (TSP desde Salida), visor interactivo de ruta completa para 25+ paradas en mapa Leaflet y enlaces inteligentes desglosados para Google Maps por tramos.
...
- `/confirmar?guia=ID`: Portal de autoservicio del cliente con 2 métodos de confirmación satelital (GPS Satelital 1-toque y Pegado de Coordenadas Universales: decimales, DMS, enlaces de Google Maps/Waze, o Plus Codes OLC) con validación geográfica en tiempo real y selector de hora disponible.
- `/api/guias`, `/api/guias/[id]/foto`, `/api/confirmar`, `/api/choferes`, `/api/chofer/guias`, `/api/auth/login`, `/api/escaneos`.
